...
Cube.prototype.draw = function(ctx, x, y) {
  ...
  // up face
  ctx.save();
  drawParallelogram(fillColors[2], ctx, 60, 0, -20, 40);
  ctx.restore();
  
  // right face
  ctx.save();
  ctx.translate(60,0);
  drawParallelogram(fillColors[1], ctx, 0, 60, -20, 40);
  ctx.restore();

  // front face
  ctx.save();
  ctx.translate(-20,40);
  drawParallelogram(fillColors[0], ctx, 60, 0, 0, 60);
  ctx.restore();

  ctx.restore();
}

drawParallelogram = function(color, ctx, x1, y1, x2, y2) {
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.moveTo(0, 0);
  ctx.lineTo(x1, y1);
